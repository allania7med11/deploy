...
sudo -u postgres psql shop_db < /vagrant/apps/shop_back/shop_db_init.sql
echo "****Load and run shop_back image****"
docker pull allania7med11/shop_back:latest
sudo kill -9 $(sudo lsof -t -i:8000)
sudo docker run -d  --network=host --env-file /vagrant/config/shop_back/.env --restart unless-stopped shop_back:latest 8000
